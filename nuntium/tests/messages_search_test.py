...
class MessagesSearchQuerysetTestCase(TestCase):
    def setUp(self):
        super(MessagesSearchQuerysetTestCase, self).setUp()
    ...
    def test_public_messages_are_searchable(self):
        public_messages = list(Message.objects.filter(public=True))
        from ..search_indexes import message_search_queryset
        qs = list(message_search_queryset())
        for msg in public_messages:
            self.assertIn(msg, qs)
    ...
    def test_it_does_not_search_within_private_messages(self):
        ...
        from ..search_indexes import message_search_queryset
        self.assertNotIn(message, message_search_queryset())
    ...
    def test_it_does_not_search_within_non_confirmated_messages(self):
        ...
        from ..search_indexes import message_search_queryset
        qs = message_search_queryset()
        self.assertIn(confirmated_message, qs)
        self.assertNotIn(non_message, qs)
...
class MessageSearchViewTestCase(TestCase):
    ...
    def test_search_view(self):
        self.assertTrue(issubclass(MessageSearchView, View))
        self.assertEquals(MessageSearchView.template_name, 'nuntium/search.html')
...
class PerInstanceSearchFormTestCase(SearchIndexTestCase):
    ...
    def test_per_instance_search_view(self):
        self.assertTrue(issubclass(PerInstanceSearchView, View))
        self.assertEquals(PerInstanceSearchView.template_name, 'nuntium/instance_search.html')
